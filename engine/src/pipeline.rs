...
impl TpuPipeline {
    ...
    pub async fn process_batch(&self, bundles: Vec<Bundle>, loose_txs: Vec<Transaction>) -> Result<()> {
        let units = self.prioritizer.priortize(bundles.clone(), loose_txs.clone());
        println!("Prioritized {} units", units.len());

        let mut bundle_map: HashMap<u64, Bundle> = bundles.into_iter().map(|b| (b.id, b)).collect();

        let mut tx_map: HashMap<String, Transaction> = loose_txs.into_iter().map(|tx| (tx.signature.clone(), tx)).collect();
        ...
        let assignments = self.scheduler.schedule(bundles_for_schedule, txs_for_schedule)?;
